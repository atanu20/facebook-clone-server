    ...
    // console.log(targetFile)
        ...
        targetFile.mv(uploadDir, async (err) => {
            if (err)
            {
                ...
                const usedet=new userPostTable({postdesc:req.body.postdesc,postimg:imgname,user:req.body.user, userproimg:req.body.userproimg, user_id:req.body.user_id})
...
router.post("/updateprofile",async(req,res)=>{
    try{
        let targetFile = req.files.coverPicture;
        let profileFile = req.files.profilePicture;
        
        // console.log(targetFile)
            let extName = path.extname(targetFile.name);
            let proName = path.extname(profileFile.name);
           
        
           
            const num= random.int(10000,999999);
            let uploadDir = path.join(__dirname, 'public','images','profile', num+"_"+targetFile.name);
            let prouploadDir = path.join(__dirname, 'public','images','profile', num+"_"+profileFile.name);
            ...
            targetFile.mv(uploadDir,  async(err) => {
               try{
                ...
                profileFile.mv(prouploadDir, async (err) =>{
                    try{
                     const covername=num+"_"+targetFile.name;
                     const proname=num+"_"+profileFile.name;
                 
                        const result=await userProfileTable.findOne({userID:req.body.userId})

                    const ress= await userProfileTable.findByIdAndUpdate(result._id, { 
                         profilePicture:proname,
                     ...
                     allpost.map(async(val)=>{
                        await userPostTable.findByIdAndUpdate(val._id,{
                            userproimg :proname
                     ...
                     almsg.map(async(val)=>{
                        await msgTable.findByIdAndUpdate(val._id,{
                            profile_image:proname
...
router.post("/postmsg",async(req,res)=>{
    try{
        ...
        const updatedPost = await userPostTable.findByIdAndUpdate({_id:req.body.post_id}, { totalmsg: post.totalmsg + 1 }, { new: true });

        const usedet=new msgTable({uname:req.body.name,profile_image:req.body.profile_image,post_id:req.body.post_id, message:req.body.message, usid:req.body.usid})
        const resu= await usedet.save();
        res.json({submit:true})
    }
    catch(err){
...
router.post("/follow",async(req,res)=>{
    try{
        ...
        const usedet=new followingTable({u_id:req.body.u_id, following_id:req.body.following_id})
        const resu= await usedet.save();
        res.json({submit:true})
    }
    catch(err){
...
router.post("/unfollow",async(req,res)=>{
    try{
        ...
        const result=await followingTable.find({u_id:req.body.u_id, following_id:req.body.following_id})
...
router.get('/getFollowers/:uid',async(req,res)=>{
    try{
        // const userPosts=[]
        const fdresult=await followingTable.find({following_id:req.params.uid})
        //  const result=await followingTable.find({u_id:req.params.uid})
...
router.post("/like",async(req,res)=>{
    try{
        ...
        const updatedPost = await userPostTable.findByIdAndUpdate({_id:req.body.postid}, {likes: post.likes + 1 }, { new: true });
        const usedet=new likeTable({uid:req.body.uid, postid:req.body.postid})
        const resu= await usedet.save();
        res.json({submit:true})
    }
    catch(err){
...
router.post("/checklike",async(req,res)=>{
    try{
        const result = await likeTable.find({uid:req.body.uid,postid:req.body.postid});
        if(result.length)
        {
            res.json({submit:true})
        }
